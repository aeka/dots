...
#sep_b="%{B#FF222427}%{F#FF917154}  %{F-}%{B-}"
sep_b="%{B#FF222427}%{F#FF917154}  %{F-}%{B-}"
...
{   
    ...
    # battery
    while true ; do
	echo "bat $(acpi -b | awk "{print $1}" | sed 's/\([^:]*\): \([^,]*\), \([0-9]*\)%.*/\3/')%"
	sleep 1 || break
    ...
    #    echo "vol $(amixer get PCM | tail -1 | sed 's/.*\[\([0-9]*%\)\].*/\1/')%"
        echo "vol $(amixer get Master -M | grep -oE "[[:digit:]]*%")"
	sleep 1 || break
    done > >(uniq_linebuffered) &
    vol_pid=$!
...
} 2> /dev/null | lemonbar -u 2 -g x12 -B '#FF1A1C1F' -F '#FFA8A8A8' -f '*-stlarch-medium-r-*-*-10-*-*-*-*-*-*-*,-*-cure.se-medium-r-*-*-11-*-*-*-*-*-*-*' $1
